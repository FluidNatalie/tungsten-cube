data:extend({
 	{
 		type = "recipe",
 		name = "tungsten-cube-recipe",
		subgroup = "cubes-subgroup",
 		category = "crafting",
 		energy_required = 3600,
 		ingredients = {
 			{ type = "item", name = "tungsten-plate", amount = 500 },
 		},
 		results = {
 			{ type = "item", name = "tungsten-cube", amount = 1 },
 		},
 		hide_from_signal_gui = false,
 		auto_recycle = true,
        enabled = false
    },
	{
 		type = "recipe",
 		name = "biter-egg-cube-recipe",
		subgroup = "cubes-subgroup",
 		category = "crafting",
 		energy_required = 300,
 		ingredients = {
 			{ type = "item", name = "biter-egg", amount = 500 },
 		},
 		results = {
 			{ type = "item", name = "biter-egg-cube", amount = 1 },
 		},
 		hide_from_signal_gui = false,
 		auto_recycle = true,
        enabled = false
    },
	{
 		type = "recipe",
 		name = "carbon-cube-recipe",
		subgroup = "cubes-subgroup",
 		category = "crafting",
 		energy_required = 300,
 		ingredients = {
 			{ type = "item", name = "carbon", amount = 500 },
 		},
 		results = {
 			{ type = "item", name = "carbon-cube", amount = 1 },
 		},
 		hide_from_signal_gui = false,
 		auto_recycle = true,
        enabled = false
    },
	{
 		type = "recipe",
 		name = "copper-cube-recipe",
		subgroup = "cubes-subgroup",
 		category = "crafting",
 		energy_required = 600,
 		ingredients = {
 			{ type = "item", name = "copper-plate", amount = 1000 },
 		},
 		results = {
 			{ type = "item", name = "copper-cube", amount = 1 },
 		},
 		hide_from_signal_gui = false,
 		auto_recycle = true,
        enabled = false
    },
	{
 		type = "recipe",
 		name = "holmium-cube-recipe",
		subgroup = "cubes-subgroup",
 		category = "crafting",
 		energy_required = 1800,
 		ingredients = {
 			{ type = "item", name = "holmium-plate", amount = 500 },
 		},
 		results = {
 			{ type = "item", name = "holmium-cube", amount = 1 },
 		},
 		hide_from_signal_gui = false,
 		auto_recycle = true,
        enabled = false
    },
	{
 		type = "recipe", 
 		name = "iron-cube-recipe",
		subgroup = "cubes-subgroup",
 		category = "crafting",
 		energy_required = 600,
 		ingredients = {
 			{ type = "item", name = "iron-plate", amount = 1000 },
 		},
 		results = {
 			{ type = "item", name = "iron-cube", amount = 1 },
 		},
 		hide_from_signal_gui = false,
 		auto_recycle = true,
        enabled = false
    },
	{
 		type = "recipe",
 		name = "u-235-cube-recipe",
		subgroup = "cubes-subgroup",
 		category = "crafting",
 		energy_required = 600,
 		ingredients = {
 			{ type = "item", name = "uranium-ore", amount = 500 }, --recipe is subject to change
 		},
 		results = {
 			{ type = "item", name = "u-235-cube", amount = 1 },
 		},
 		hide_from_signal_gui = false,
 		auto_recycle = true,
        enabled = true --set to false once a tech has been added
    },
})